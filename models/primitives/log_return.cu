...
template <typename T, int ELEMENTS_PER_BLOCK>
__global__ void log_return_kernel(const T* __restrict__ input, T* __restrict__ output, int n) {
    int gid = (blockIdx.x * blockDim.x + threadIdx.x) * ELEMENTS_PER_BLOCK;
...
template <typename T, int BLOCK_SIZE, int ELEMENTS_PER_BLOCK>
void log_return(const T* input, T* output, int n) {
    const int TILE_SIZE = BLOCK_SIZE * ELEMENTS_PER_BLOCK;
    ...
    log_return_kernel<T, ELEMENTS_PER_BLOCK>
